...
| A17 | **One click: add Google Drive to the `Market Comps — daily check across every market` Routine.** It fires at 22:00 UTC and enumerates every market folder under Drive `Comps`, compares each one's published as-of against what has arrived, and refreshes anything the 21:30 workflow missed. It cannot do any of that today: this org does not allow a Routine to be created with a connector attached (`create_trigger` refuses the parameter), so every firing reaches its own guard, says it has no Drive tools, and stops. Add the connector from the claude.ai Routines UI and it starts working with no other change. **Nothing is broken meanwhile** — `.github/workflows/refresh_comps.yml` does the actual daily refresh with the pipeline's own service account and needs nothing added to it; the Routine is the second pair of eyes, and the thing that would notice a market the workflow's folder scan cannot see. Notifications are off on it until it works, so it is silent rather than noisy | Whether a missed market gets noticed by anything other than a person looking | no — the workflow publishes the tab either way |
