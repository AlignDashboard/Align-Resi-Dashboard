...
| A18 | **The Rental Rates tab is a snapshot, refreshed by hand.** It holds the Rental Rate Tracker's data as of 2026-09-23, encrypted under the tracker's own password; `scripts/import_rental_tracker.py` refreshes it from a clone of `github.com/dbalduc/rental-rates`, which its owner rebuilds daily. Automating that is a small workflow on the `refresh_comps.yml` pattern plus the tracker's password as an Actions secret (`RENTAL_TRACKER_PASSWORD`) — your call whether that password should live in this repo's secrets | A Rental Rates tab that keeps up with the tracker | no — the tab states its snapshot date and turns it red past a week |
