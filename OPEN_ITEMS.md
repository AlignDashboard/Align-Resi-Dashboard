...
| A13 | **Two secrets to let the filing script deploy itself**, if you want the paste automated: `CLASPRC_JSON` (from `clasp login` as `dashboard@alignrealestate.com` — an OAuth refresh token, so a real credential) and `APPS_SCRIPT_ID`, plus switching the Apps Script API on once at `script.google.com/home/usersettings`. `deploy_filing_script.yml` is written, tested and already running as a validate-only no-op until they exist. A service account cannot do this — `projects.updateContent` needs an end-user token. Running `previewRouting`/`resortExistingFiles` remotely is deliberately **not** wired: it would need a token carrying Gmail-read and Drive-write, and the dry-run log is the safety net | Whether a routing change reaches Drive without a manual paste | no — the workflow validates and stands down without them |
...
| # | Item | Detail |
| --- | --- | --- |
| C3 | `_Unsorted` holds 22 files. **The two the pipeline can read are no longer stuck**: `fetch_drive` now rescues them by filename on the next cron run, so the funnel data flows without anyone touching Apps Script. The remaining 20 have no parser, so tidying them is cosmetic — paste `gmail_drive_filing.js` into the `file downloader` project and run `previewRouting` → `checkFolders` → `resortExistingFiles` when convenient, and they sort into named folders. No longer urgent, and no longer blocking any number on the page |
