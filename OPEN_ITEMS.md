...
State as of 2026-09-21. IDs are stable — when an item closes, move
...
| C9 | **The comp exports file themselves, because the deployed filer predates the `Comps` rule — this is A16's first measurable consequence.** The rule (`folder: 'Comps'`) was committed 2026-09-18T01:20Z, sixteen days after the last successful deploy; eight minutes later the live filer auto-derived `HelloData Simple` and `HelloData Full` at the top of the drop tree, and files arriving 2026-09-21 still land there. So A16 is no longer only a broken workflow — a routing rule written since 2026-09-02 has never taken effect, and this is what that looks like from Drive. **Nothing is broken by it**: the rescue sweep claims those files by `name_patterns` (`*hellodata*`), the pipeline reads them, and `store_comps` picks the newest vintage wherever they sat. What decays is the tidy tree — the 2026-09-21 reorganisation moved those two folders into `Comps/<market>/Simple`, so the next arrival recreates them empty at the top level and the vintages split across two places. Two separate things to decide: deploy the script at all, and then whether the filer should file into `Comps/San Francisco/Simple` rather than `Comps/` — it cannot today, because `getSubfolder_` creates a single folder and `test_routing.py` forbids a `/` in a rule's folder name (a `/` is legal in a Drive folder name, which is the accident that check exists to prevent). Making nesting first-class means teaching `getSubfolder_` to walk a path and changing that check from "no `/`" to "every segment is real" |
