...
def data():
    return {
        "snapshot": "2026-09-23",
        "leases": [
            {"date": "2026-05-10", "property": "The Landing", "unit": "652", "sqft": 984,
             "grossRent": 7155.0, "grossPsf": 7.27, "netRent": 7155.0, "netPsf": 7.27,
             "prior": 4700.0, "toPct": 52.2, "toDollars": 2455.0, "term": "12"},
            {"date": "2026-06-05", "property": "Brand New", "unit": "042", "sqft": 600,
             "grossRent": 3300.0, "grossPsf": 5.5, "netRent": 3300.0, "netPsf": 5.5,
             "prior": 2875.0, "toPct": 14.8, "toDollars": 425.0, "term": None},
        ],
        "renewals": [
            {"date": "2026-05-02", "property": "The Landing", "unit": "213", "sqft": 799,
             "oldRate": 4761.0, "newRate": 5189.0, "incPct": 9.0,
             "note": f"corrected per tracker (tenant {NAME}), confirmed by JW"},
            {"date": "2026-06-01", "property": "The Landing", "unit": "214", "sqft": None,
        ...
        "weekly": [
            {"week": "5/25–5/31", "ws": "2026-05-25", "we": "2026-05-31",
             ...
             "occ": 95.82, "leased": 98.1},
        ],
        "mtm": {"The Landing": {"asOf": "2026-09-21", "units": 27, "occupied": 260, "pct": 10.4,
...
def page(d, password=PW, extra_app=True):
    ...
    app = ('<html><h1>Rental Rate Tracker</h1><div class="sub">Chorus &amp; The Landing '
           'sourced from <b>RealPage</b>.</div><script>const DATA=' + json.dumps(d) + ';'
           + ('const props=["The Landing","Chorus"];const OCC={"Chorus":{occ:96.3,leased:98.1}};'
...
def main():
    ...
    try:
        ...
        check("the file decrypts to the tracker's own figures",
              p["leases"][0]["netRent"] == 7155.0 and p["leases"][0]["toPct"] == 52.2)
        ...
        check("changed data is re-encrypted under a fresh salt",
              ...
              and opened(out)["leases"][0]["netRent"] == 7200.0)
        snap = out.read_bytes()
        ...
        check("the tracker's stated occupancy is kept",
              p["occ_stated"] == {"Chorus": {"occ": 96.3, "leased": 98.1}})
        check("the sources line arrives as plain text",
              p["about"] == "Chorus & The Landing sourced from RealPage.", repr(p["about"]))
